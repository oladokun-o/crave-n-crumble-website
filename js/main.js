...
// Your LocationIQ API Access Token
const apiKey = 'pk.df2cd7e07f9227c6f5cce6e44377506b';
...
document.getElementById("country-select").addEventListener("change", function () {
  ...
  if (selectedCountry) {
    // Fetch states using LocationIQ API
    fetch(`https://api.locationiq.com/v1/autocomplete.php?key=${apiKey}&q=${selectedCountry}&tag=place:region&format=json`)
      .then(response => response.json())
      .then(data => {
        console.log(data); // Log the response to inspect its structure

        if (Array.isArray(data)) {
          const uniqueStates = new Set(); // Use Set to avoid duplicate states

          data.forEach(location => {
            if (location.address && location.address.state) {
              uniqueStates.add(location.address.state);
            }
          });
